...
function generateOrderNotificationEmail(data: any, lang = 'en'): string {
  ...
  const identificationType = data.identificationType || 'vin'
  const identificationValue = data.identificationValue || data.vinNumber || 'N/A'

  return `<!DOCTYPE html><html><body>
    <h1>${t['email_new_order'] || 'New Order Received'}</h1>
    <p><strong>${t['email_order_number'] || 'Order Number'}:</strong> ${data.orderNumber}</p>
    <p><strong>${t['email_customer_name'] || 'Customer Name'}:</strong> ${data.customerName || 'N/A'}</p>
    <p><strong>${t['email_customer_phone'] || 'Customer Phone'}:</strong> ${data.customerPhone || 'N/A'}</p>
    ...
    <p><strong>Identification Value:</strong> ${identificationValue}</p>
