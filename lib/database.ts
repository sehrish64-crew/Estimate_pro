...
export async function insertOrder(order: {
  ...
  try {
    ...
    // Try to insert including `customer_name`, `customer_phone`, and `state`.
    // If the database schema hasn't been migrated yet, MySQL will throw an
    // error for unknown columns; fall back through older schemas as needed.
    let result: any
    try {
      [result] = await conn.execute(
        `INSERT INTO orders (customer_email, customer_name, customer_phone, vehicle_type, identification_type, identification_value, vin_number, package_type, country_code, state, currency, amount, payment_status, payment_provider, status, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [
      ...
      if (err?.errno === 1054 || /Unknown column .* in 'field list'/.test(msg)) {
        console.warn('insertOrder: orders schema missing fields, falling back to legacy inserts')
        try {
          const [fallbackResult]: any = await conn.execute(
            `INSERT INTO orders (customer_email, customer_name, customer_phone, vehicle_type, identification_type, identification_value, vin_number, package_type, country_code, currency, amount, payment_status, payment_provider, status, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
            [
              order.customer_email,
              order.customer_name || null,
          ...
          const nestedMsg = String(nestedErr?.message || nestedErr)
          if (nestedErr?.errno === 1054 || /Unknown column .* in 'field list'/.test(nestedMsg)) {
            console.warn('insertOrder: customer_name/customer_phone not present, falling back to oldest legacy insert')
            const [legacyResult]: any = await conn.execute(
